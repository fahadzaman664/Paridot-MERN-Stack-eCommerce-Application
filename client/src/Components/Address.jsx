...
const Address = () => {
  ...
  const { cart, shippingAddress, userAddress } = useContext(AppContext);
  ...
  const handleCountryChange = (val) => {
    setShippingInfo({ ...shippingInfo, country: val, state: "" });
  };
  ...
  const { fullName, address, city, pinCode, phoneNumber, country, state } =
    shippingInfo;

  const handleSubmit = async (e) => {
    e.preventDefault();

    const response = await shippingAddress(
      fullName,
      address,
      city,
      pinCode,
  ...
  return (
    <>
      <div className="flex flex-col-reverse md:flex-row md:justify-normal md:mx-auto md:-mt-20 md:gap-4  ">
        {/*start  contact info */}
        <div>
